...
export default function ThankYou() {
  ...
  if (!order) {
    return (
      <main className="max-w-3xl mx-auto px-4 py-10">
        ...
        <Link to="/shop" className="mt-4 inline-block rounded-xl border px-4 py-2">Retour à la boutique</Link>
      </main>
    );
  }
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-10">
      ...
      <p className="mt-2 text-gray-700">Numéro de commande : <strong>{order.id}</strong></p>
      <p className="text-gray-700">Montant : <strong>{order.amount} {order.currency}</strong></p>
      <p className="text-gray-600">Méthode : {order.method} — Statut : {order.status}</p>
        ...
        {order.items.map((it) => (
          <li key={it.id} className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="text-sm text-gray-500">Qté {it.qty} — {it.price} €</div>
            </div>
          </li>
        ))}
      </ul>
